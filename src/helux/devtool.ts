import { IPlugin, IDataChangedInfo, Fn } from 'helux';
import { createStore, combineReducers } from 'redux';

let reduxStore: any = null;
const pluginName = 'HeluxPluginReduxDevtool';
const moduleInfoMap: Record<string, { name: string, state: any }> = {
  [pluginName]: { name: pluginName, state: 'init redux devtool' }, // 避免调试时再刷新浏览器出现 no store 错误导致 devtool 无法启动
};
...
export const HeluxPluginReduxDevtool: IPlugin = {
  install(ctx) {
