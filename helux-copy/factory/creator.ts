...
import { IMutateCtx, recordDataKeyForStop, getArrKey, newMutateCtx, newOpParams } from '../factory/common/util';
import { getGlobalIdInsKeys, getGlobalInternal } from '../factory/common/globalId';
import { getMarkAtomMap } from '../factory/root';
import { runFn, markExpired, recordFnDepKey, markComputing, getDepFnStats } from '../helpers/fndep';
...
import { callWatcherMutate } from './common/mutateRule';
import { parseRules, parseRawState } from './common/parse';
...
function handleOperate(opParams: IOperateParams, opts: { internal: TInternal; mutateCtx: IMutateCtx }) {
  ...
  // 可能会缩短后再记录
  if (!recordDataKeyForStop(writeKey, stopDepInfo, (key) => { writeKeys[key] = 1 })) {
...
function interveneDeps(isAdd: boolean, opts: IHanldeStateOptions) {
  ...
  if (isFn(fn)) {
    const { mutateCtx: { depKeys }, internal: { rawState, sharedKey, isDeep } } = opts;
    let state: any;
...
function execDepFnAndInsUpdater(opts: IHanldeStateOptions) {
...
interface IHanldeStateOptions extends IInnerSetStateOptions {
  state: Dict;
  internal: TInternal;
...
function handleState(opts: IHanldeStateOptions) {
  ...
  if (isAsync) { // for dangerous async mutate
...
interface IHandleDeepMutateOpts extends IInnerSetStateOptions {
  internal: TInternal;
  forAtom: boolean;
  sharedState: SharedState;
  desc?: any;
...
export function handleDeepMutate(opts: IHandleDeepMutateOpts) {
  ...
  return {
    ...
    finishMutate(partial?: Dict, customOptions?: IInnerSetStateOptions) {
      ...
      handleOpts.state = finishDraft(draft); // a structural sharing obj generated by limu
...
function triggerChange(heluxParams: IHeluxParams) {
  const { change } = heluxParams.createOptions;
  if (!isObj(change)) return;
...
function buildSharedState(heluxParams: IHeluxParams) {
  ...
  const collectDep = (valKey: string, keyPath: string[], val: any) => {
    ...
    recordFnDepKey(depKey, { sharedKey, sharedState });
