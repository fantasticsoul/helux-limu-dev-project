...
import { dedupList, isFn, isObj, isPromise, nodupPush, noop, warn, tryAlert } from '../../utils';
...
function checkResult(fnCtx: IFnCtx, result: Dict, forAtom?: boolean) {
  ...
  if (isAsync && !isAsyncTransfer) {
    ...
    if (fnKey || sharedKey) {
      throw new Error('ERR_INVALID_CALL: derive(),deriveAsync can not transfer another derived result or shared state, it will cause wrong result');
    }
  }
}
...
export function createFnCtx(options: IMapFnCtxOptions) {
  ...
  if (runAsync && (asyncType === SOURCE || asyncType === TASK)) {
    // TODO
    // computingKeyMap works for async derive fn first running
    // key: 2/a, value: ['s1', 's2]
    // useDerived check computingKeyMap to init isComputing
    fnCtx.depKeys.forEach((key) => {
      console.log('put key', key, fnCtx.fnKey);
    });
    runFn(curFnKey, { isFirstCall: true, sn: fnCtx.renderInfo.sn + 1 }).then((data: any) => {
      fnCtx.depKeys.forEach((key) => {
        console.log('del key', key, fnCtx.fnKey);
      });
