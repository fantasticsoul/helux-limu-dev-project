...
import { delFnCtx, getFnCtxByObj, computeMod } from '../../helpers/fndep';
...
export function genDerivedResult(options: IInitOptions) {
  const { deriveCtx, fn, sourceFn, fnCtx, showProcess, asyncType = ASYNC_TYPE.MAY_TRANSFER, forAtom } = options;
  let isAsync = false;
  let upstreamFnCtx: IFnCtx | null = null;
  let isCtxChanged = false;
  const scopeType: ScopeType = SCOPE_TYPE.HOOK;

  console.log(`---> fnCtx.isComputing `, fnCtx.isComputing);
  ...
  deriveCtx.input = fn;
  let needCheckComputeStatus = false;
  ...
  // TODO check computingKeyMap to reassign isComputing
  if (
    (needCheckComputeStatus && computeMod.shouldShowComputing(fnCtx.depKeys))
    || isCtxChanged
  ) {
    fnCtx.updater();
  }
