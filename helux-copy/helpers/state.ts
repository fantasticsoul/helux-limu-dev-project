...
import { isDebug, isObj, safeMapGet, getSafeNext } from '../utils';

const sharedScope = getHelp().sharedScope;
const { UNMOUNT_INFO_MAP, SHARED_KEY_STATE_MAP, STATE_SHARED_KEY_MAP, INTERMAL_MAP, BEENING_WATCHED_MAP } = sharedScope;
...
export function bindInternal(state: Dict, internal: TInternal) {
  const key = getSharedKey(state);
