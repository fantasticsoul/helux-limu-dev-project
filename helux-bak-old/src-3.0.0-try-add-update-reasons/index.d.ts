...
*/
import type {
  Dict, DictN, EffectCb, ICreateOptionsType, SharedObject, PlainObject,
  ICreateDeepOptions, ModuleName, Mutable, NextState, IFnParams, IAsyncTaskParams,
} from './types';
...
export function createShared<T extends Dict = Dict>(
  ...
  call: <A extends any[] = any[]>(
    srvFn: (ctx: {
      args: A;
      state: T;
      setState: (partialState: Partial<T> | ((mutable: Mutable<T>) => Partial<T> | void)) => NextState;
...
export function createDeepShared<T extends Dict = Dict>(
  rawState: T | (() => T),
  createOptions?: ModuleName | ICreateDeepOptions,
): {
  state: SharedObject<T>;
  call: <A extends any[] = any[]>(
    srvFn: (ctx: {
      args: A;
      state: T;
      setState: (recipe: (mutable: Mutable<T>) => Partial<T> | void) => NextState;
    }) => Promise<Partial<T>> | Partial<T> | void,
    ...args: A
  ) => void;
