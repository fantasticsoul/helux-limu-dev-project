...
export function runFn(fnKey: string, options?: { force?: boolean; isFirstCall?: boolean, updateReasons?: TriggerReason[] }) {
  const { isFirstCall = false, updateReasons = [] } = options || {};
  const fnCtx = getFnCtx(fnKey);
  if (!fnCtx) {
    return;
  }
  ...
  const fnParams = { isFirstCall, prevResult: fnCtx.result, updateReasons };
  if (!isAsync) {
...
export function rerunComputedFn<T extends Dict = Dict>(result: T): T {
...
export function runComputed<T extends Dict = Dict>(result: T): T {
  return rerunComputedFn(result);
}

export function runAsyncComputed<T extends Dict = Dict>(result: T): Promise<T> {
  return Promise.resolve(rerunComputedFn(result));
}
