import { EXPIRE_MS, FN_KEY, NOT_MOUNT, PROTO_KEY, RENDER_START, SHARED_KEY, SIZE_LIMIT, UNMOUNT, DERIVE, ASYNC_TYPE } from '../consts';
...
export function buildFnCtx(specificProps?: Partial<IFnCtx>): IFnCtx {
  const base: IFnCtx = {
    ...
    asyncType: 'normal',
    subscribe: (cb) => {
      if (isDebug()) { // for hot update working well
        console.error('set isExpred true in subscribe');
        base.isExpred = true;
        base.updater();
      }
      cb();
